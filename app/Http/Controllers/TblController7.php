...
class TblController7 extends Controller
{
    ...
    public function addServiceCharge() {
        ...
        //dd($service_charge);
        Session::put('service7', $service_charge);
        Session::save();
    ...
    public function getepozcart(Request $request, $id)
    {
        ...
        $oldCart = Session::has('tbl7') ? Session:: get('tbl7') : null;
...
//            return redirect()->route('cart');
    ...
    public function reduceFood($id) {
        ...
        if(Session::has('tbl7')){
            ...
            Session::save();
            $totalPrice=0;
            return redirect('tblindex7')->with('totalPrice', $totalPrice);
    ...
    {
        $request->session()->forget('tbl7');
        $request->session()->forget('paid7');
        $request->session()->forget('change7');
        $request->session()->forget('open7');
        $request->session()->forget('extra7');
         $request->session()->forget('person7');
         $request->session()->forget('service7');
        $request->session()->forget('cash7');
        $request->session()->forget('card7');
    ...
    public function card()
    {
        ...
        $ordered_food_name = Session::get('name_items7');
        $price = Session::get('price_items7');
        $qty = Session::get('qty_items7');
        ...
        Session::put('card7','Card');
        Session::save();
        session()->forget(['cash7']);
        $oldCart = Session::get('tbl7');
    ...
    $qty = Session::get('qty_items7');
     ...
     $dis_p = DB::table('percent_dis')->where('id','=', 1)->value('percent');
        ...
        Session::put('cash7','Cash');
        Session::save();
        session()->forget(['card7']);
        $oldCart = Session::get('tbl7');
    ...
    public function paidStore(Request $request)
    {
        ...
        // Session::save();
        $newpay= Session::get('paid7');
        //dd($newpay);
        if($newpay){
            $change= $newpay-$total;
            Session::put('change7', $change);
            Session::save();
            TblController7::cash();
    ...
    public function index()
    {
        ...
        return redirect('tpoz7/13')->with('discount','foods','tax','open','extra','foods','last');

        //return view('epoz/tblcart7',compact('discount','foods','tax','open','extra','foods','last'));
        // return redirect('epoz');
    }
