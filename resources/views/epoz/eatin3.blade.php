...
@foreach($products as $product)
    ...
    <td class="pl-1 pt-1 pr-1 m-0" style="width:20px; height: 20px;">
         <a href="{{route('lessfood3',['id'=>$product['item']['id']])}}" type="button" class="m-btn px-3 py-1 btn btn-danger"><i class="fa fa-times" aria-hidden="true"></i></a>
    </td>
...
<figure class="media pt-2">
	...
	<td class="py-0" class="text-center">
		...
		<a href="{{route('less3',['id'=>$product['item']['id']])}}" type="button" class="m-btn btn px-3 btn-primary mt-1"><i class="fa fa-minus"></i></a>
		<button type="button" class="m-btn btn btn-default mt-1" disabled>{{$product['qty']}}</button>
		<a href="{{route('moreCart3',['id'=>$product['item']['id']])}}}" type="button" class="m-btn btn px-3 btn-primary mt-1"><i class="fa fa-plus"></i></a>
		</div>
	</td>
	<td  class="py-0" style="height:20px;">
...
<div class="box">
    ...
    $open = Session::get('open3');
    $tbl = Session::get('tbl_no3');
    $person = Session::get('person3');

    $dis = epozDiscount::find(1);
    $extra = Session::get('extra3');
    $oldCart = Session::get('tbl3');
    $cart = new epozCart($oldCart);
    $products = $cart->items;
...
@if(Session::has('tbl3'))
    <dl class="dlist-align">
        ...
        <?php
        use App\Paid;
        $pay = Session::get('paid3');  
        $changes = Session::get('change3');
...
<button type="button" class="btn btn-primary  py-3 btn-lg btn-block" data-toggle="modal" data-target="#exampleModalCenter3">
    <i class="far fa-money-bill-alt "></i> Cash
  </button>
  
  <!-- Modal -->
  <div class="modal" id="exampleModalCenter3" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
...
<div class="btn-group-vertical w-100" role="group" aria-label="Basic example">
    <div class="">
        ...
        <form method="post" action="{{route('tblpaid3')}}">
            @csrf
            @method('post')
            <div class="form-group">
                <input class="text-center form-control-lg myinput" id="code3"  name="paid" style="border-style: none; font-size:30px;" placeholder="Enter Cash amount">
            @if(session::has('tbl3'))
    ...
    <div class="btn-group">
        ...
        <button type="button" class="btn btn mynmbbtn" onclick="document.getElementById('code3').value=document.getElementById('code3').value + '3';">3</button>
    </div>
    <div class="btn-group">
    ...
    <button type="submit" class="btn mt-3 btn-success mysavebtn" onclick="">Paid</button>
        ...
        <div class="col-md-6 mb-0">
            <a href="{{route('tblcard3')}}" class="btn py-3 btn-warning btn-lg btn-block"><i class="fa fa-credit-card"></i> Card </a>
        </div>
    </div>
    ...
    <div class="row pt-0 mt-0 mb-0">
        ...
        <input type="button" value="Print" onclick="PrintDiv3();" id="btnPrint" class="btn py-3 btn-info btn-lg btn-block"/>
            </div>
        <div class="col-md-6 pt-0 mt-0">
            <a href="{{route('destroy3')}}" class="btn py-3  btn-danger btn-lg btn-block"><i class="fa fa-times-circle "></i> Clear Cart </a>
        </div>
    </div>
    ...
    <script type="text/javascript">
        function PrintDiv3() {
            var contents = document.getElementById("printContent3").innerHTML;
