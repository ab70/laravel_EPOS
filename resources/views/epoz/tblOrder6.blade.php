...
<?php
    ...
    if ( ! Session::has('tbl6')){
        ...
        ?>
            ...
            <div class="col">
            <a href="{{route('index6')}}">
            <button class="btn btn-warning btn-sm px-1 py-3 m-0"   style="width: 70px;font-size:20px;" data-target="#lev6">6</button>
            </a>
            <?php
            if (Session::has('tbl6')){?>
                <div class="mt-1 card bg-danger" style="width:70px;height:10px;"></div>
...
<div class="btn-group-vertical w-200" role="group" aria-label="Basic example">
    <div class="">
        ...
        <form method="post" id="form2" action="{{route('order_tbl6')}}">
            @csrf
                  @method('post')
...
<figure class="media pt-2">
  ...
  <?php
       if($product['item']->name){
        $name_array[]= $product['item']->name;
         Session::put('name_items6',$name_array);
         Session::save();
	}else{
	    $name_array ='';
	}
 ...
 <?php
       if($product['price']){
        $price[] = $product['price'];
         Session::put('price_items6',$price);
        Session::save();
	}else{
	    $price="";
...
<dl class="dlist-align">
    ...
    <dl class="dlist-align">
        ...
        <dd class="text-right">-£{{number_format($extra,2)}}</dd>
                    ...
                    @if(session::has('card6'))
                <?php $card  = Session::get('card6') ?>
    <dl class="dlist-align">
         <dt class="b" >Payment:</dt>
        <dd class="text-right text-warning b"> {{$card}} </dd>
        @endif
    </dl>
    @if(session::has('cash6'))
                <?php $cash = Session::get('cash6') ?>
    <dl class="dlist-align">
        <dt class="b" >Payment:</dt>
...
@if(Session::has('tbl6'))
    <dl class="dlist-align">
        ...
        @endif
         ...
         @if(Session::has('service6')&& Session::has('tbl6'))
<?php     $service = Session::get('service6');
 ?>
<dl class="dlist-align">
...
<div class="btn-group-vertical w-100" role="group" aria-label="Basic example">
    ...
    <div class="row pt-1 mt-0 mb-0 ">
        {{-- <div class="col-md-6 pt-0 mt-0 mb-0">
            <a href="{{route('printPos6')}}" class="btn py-3 btn-info btn-lg btn-block"><i class="fa fa-print"></i> Print </a>
        </div> --}}
...
<input type="button" value="Inst.P" onclick="PrintDiv6();" id="btnPrint" class="btn py-5 btn-success btn-lg btn-block"/>
</div>
</div>
