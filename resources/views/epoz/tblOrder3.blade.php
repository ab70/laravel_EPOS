...
<div class="btn-group-vertical w-200" role="group" aria-label="Basic example">
    <div class="">
        ...
        <form method="post" id="form2" action="{{route('order_tbl3')}}">
            @csrf
                  @method('post')
  ...
  <?php
      if($product['item']->name){
        $name_array[]= $product['item']->name;
         Session::put('name_items3',$name_array);
         Session::save();
	}else{
	    $name_array ='';
	}
	...
	<?php
       if($product['price']){
        $price[] = $product['price'];
         Session::put('price_items3',$price);
     Session::save();
	}else{
	$price="";
	}
...
<dl class="dlist-align">
    ...
    <dl class="dlist-align">
        ...
        <dd class="text-right">-£{{number_format($extra,2)}}</dd>
            ...
            @if(session::has('card3'))
                <?php $card  = Session::get('card3') ?>
    <dl class="dlist-align">
         <dt class="b" >Payment:</dt>
        <dd class="text-right text-warning b"> {{$card}} </dd>
        @endif
    </dl>
    @if(session::has('cash3'))
                <?php $cash = Session::get('cash3') ?>
    <dl class="dlist-align">
        <dt class="b" >Payment:</dt>
...
@if(Session::has('tbl3'))
    <dl class="dlist-align">
        ...
        @endif
         ...
         @if(Session::has('service3')&& Session::has('tbl3'))
<?php     $service = Session::get('service3');
 ?>
<dl class="dlist-align">
...
<div class="btn-group-vertical w-100" role="group" aria-label="Basic example">
    ...
    <div class="row pt-1 mt-0 mb-0 ">
        {{-- <div class="col-md-6 pt-0 mt-0 mb-0">
            <a href="{{route('printPos3')}}" class="btn py-3 btn-info btn-lg btn-block"><i class="fa fa-print"></i> Print </a>
        </div> --}}
...
<input type="button" value="Inst.P" onclick="PrintDiv3();" id="btnPrint" class="btn py-5 btn-success btn-lg btn-block"/>
</div>
</div>
