...
<?php
        use App\epozCart;
    if ( ! Session::has('tbl4')){
        return view ('epoz.tblOrder4',['products'=>NULL]);
    }
        ...
        ?>
            ...
            <div class="col">
            <a href="{{route('index4')}}">
            <button class="btn btn-warning btn-sm px-1 py-3 m-0"   style="width: 70px;font-size:20px;" data-target="#lev4">4</button>
            </a>
            <?php
            if (Session::has('tbl4')){?>
                <div class="mt-1 card bg-danger" style="width:70px;height:10px;"></div>
...
<div class="btn-group-vertical w-200" role="group" aria-label="Basic example">
    <div class="">
        ...
        <form method="post" id="form2" action="{{route('order_tbl4')}}">
            @csrf
                  @method('post')
...
@foreach($products as $product)
    ...
    <td class="pl-1 pt-1 pr-1 m-0" style="width:20px; height: 20px;">
         <a href="{{route('lessfood4',['id'=>$product['item']['id']])}}" type="button" class="m-btn px-3 py-1 btn btn-danger"><i class="fa fa-times" aria-hidden="true"></i></a>
    </td>
...
<figure class="media pt-2">
	  ...
	  <?php
       if($product['item']->name){
        $name_array[]= $product['item']->name;
         Session::put('name_items4',$name_array);
         Session::save();
	}else{
	    $name_array ='';
	}
	...
	<td class="py-0" class="text-center">
		...
		<a href="{{route('less4',['id'=>$product['item']['id']])}}" type="button" class="m-btn btn px-3 btn-primary mt-1"><i class="fa fa-minus"></i></a>
		<button type="button" class="m-btn btn btn-default mt-1" disabled>{{$product['qty']}}</button>
		<a href="{{route('moreCart4',['id'=>$product['item']['id']])}}}" type="button" class="m-btn btn px-3 btn-primary mt-1"><i class="fa fa-plus"></i></a>
		</div>
	</td>
	 <?php
	...
	<td  class="py-0" style="height:20px;">
	 ...
	 <?php
       if($product['price']){
        $price[] = $product['price'];
        Session::put('price_items4',$price);
        Session::save();
	}else{
	$price="";
	}
...
<dl class="dlist-align">
    ...
    <dl class="dlist-align">
        ...
        <dd class="text-right">-£{{number_format($extra,2)}}</dd>
                      ...
                      @if(session::has('card4'))
                <?php $card  = Session::get('card4') ?>
    <dl class="dlist-align">
         <dt class="b" >Payment:</dt>
        <dd class="text-right text-warning b"> {{$card}} </dd>
        @endif
    </dl>
    @if(session::has('cash4'))
                <?php $cash = Session::get('cash4') ?>
    <dl class="dlist-align">
        <dt class="b" >Payment:</dt>
...
@if(Session::has('tbl4'))
    <dl class="dlist-align">
        ...
        @endif
         ...
         @if(Session::has('service4')&& Session::has('tbl4'))
<?php     $service = Session::get('service4');
 ?>
<dl class="dlist-align">
...
<div class="btn-group-vertical w-100" role="group" aria-label="Basic example">
    <div class="">
        ...
        <form method="post" action="{{route('tblpaid4')}}">
            @csrf
            @method('post')
            <div class="form-group">
    ...
    <button type="submit" class="btn mt-3 btn-success mysavebtn" onclick="">Paid</button>
        ...
        <div class="col-md-6 mb-0 pl-1">
            <a href="{{route('tblcard4')}}" class="btn py-3 btn-warning btn-lg btn-block"><i class="fa fa-credit-card"></i> Card </a>
        </div>
    </div>

    <div class="row pt-1 mt-0 mb-0 ">
        {{-- <div class="col-md-6 pt-0 mt-0 mb-0">
            <a href="{{route('printPos4')}}" class="btn py-3 btn-info btn-lg btn-block"><i class="fa fa-print"></i> Print </a>
        </div> --}}

            <div class="col-md-6 pt-0 mt-0 pr-1">
        <input type="button" value="Print" onclick="PrintDiv4();" id="btnPrint" class="btn py-3 btn-info btn-lg btn-block"/>
            </div>
        <div class="col-md-6 pt-0 mt-0 pl-1">
            <a href="{{route('destroy4')}}" class="btn py-3  btn-danger btn-lg btn-block"><i class="fa fa-times-circle "></i> Clear Cart </a>
        </div>
    </div>

    </div>
<div class="col-md-2 px-1">
<input type="button" value="Inst.P" onclick="PrintDiv4();" id="btnPrint" class="btn py-5 btn-success btn-lg btn-block"/>
</div>
</div>
    ...
    <script type="text/javascript">
        function PrintDiv4() {
            var contents = document.getElementById("printContent4").innerHTML;
