...
<?php
    ...
    if ( ! Session::has('tbl5')){
        ...
        ?>
            ...
            <div class="col">
            <a href="{{route('index5')}}">
            <button class="btn btn-warning btn-sm px-1 py-3 m-0"  style="width: 70px;font-size:20px;" data-target="#lev5">5</button>
            </a>
            <?php
            if (Session::has('tbl5')){?>
                <div class="mt-1 card bg-danger" style="width:70px;height:10px;"></div>
...
<div class="btn-group-vertical w-200" role="group" aria-label="Basic example">
    <div class="">
        ...
        <form method="post" id="form2" action="{{route('order_tbl5')}}">
            @csrf
                  @method('post')
...
<figure class="media pt-2">
	...
	<?php
       if($product['item']->name){
        $name_array[]= $product['item']->name;
         Session::put('name_items5',$name_array);
         Session::save();
	}else{
	    $name_array ='';
	}
	...
	<?php
       if($product['price']){
        $price[] = $product['price'];
         Session::put('price_items5',$price);
     Session::save();
	}else{
	$price="";
	}
...
<dl class="dlist-align">
    ...
    <dl class="dlist-align">
        ...
        <dd class="text-right">-£{{number_format($extra,2)}}</dd>
                    ...
                    @if(session::has('card5'))
                <?php $card  = Session::get('card5') ?>
    <dl class="dlist-align">
         <dt class="b" >Payment:</dt>
        <dd class="text-right text-warning b"> {{$card}} </dd>
        @endif
    </dl>
    @if(session::has('cash5'))
                <?php $cash = Session::get('cash5') ?>
    <dl class="dlist-align">
        <dt class="b" >Payment:</dt>
...
@if(Session::has('tbl5'))
    <dl class="dlist-align">
        ...
        @endif
         ...
         @if(Session::has('service5')&& Session::has('tbl5'))
<?php     $service = Session::get('service5');
 ?>
<dl class="dlist-align">
...
<input type="button" value="Inst.P" onclick="PrintDiv5();" id="btnPrint" class="btn py-5 btn-success btn-lg btn-block"/>
</div>
</div>
