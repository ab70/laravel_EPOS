...
<?php
        use App\epozCart;
    if ( ! Session::has('tbl2')){
        return view ('epoz.tblOrder2',['products'=>NULL]);
    }
        ...
        ?>
            ...
            <div class="col">
            <a href="{{route('index2')}}">
            <button class="btn btn-warning btn-sm px-1 py-3 m-0"    style="width: 70px;font-size:20px;" data-target="#lev2">2</button>
            </a>
            <?php
            if (Session::has('tbl2')){?>
                <div class="mt-1 card bg-danger" style="width:70px;height:10px;"></div>
...
<div class="btn-group-vertical w-200" role="group" aria-label="Basic example">
    <div class="">
        ...
        <form method="post" id="form2" action="{{route('order_tbl2')}}">
            @csrf
                  @method('post')
...
@foreach($products as $product)
    ...
    <td class="pl-1 pt-1 pr-1 m-0" style="width:20px; height: 20px;">
         <a href="{{route('lessfood2',['id'=>$product['item']['id']])}}" type="button" class="m-btn px-3 py-1 btn btn-danger"><i class="fa fa-times" aria-hidden="true"></i></a>
    </td>
    <?php
...
<figure class="media pt-2">
	...
	<?php
        ...
        $name_array[]= $product['item']->name;
         Session::put('name_items2',$name_array);
         Session::save();
	}else{
	    $name_array ='';
	}
	...
	<td class="py-0" class="text-center">
		...
		<a href="{{route('less2',['id'=>$product['item']['id']])}}" type="button" class="m-btn btn px-3 btn-primary mt-1"><i class="fa fa-minus"></i></a>
		<button type="button" class="m-btn btn btn-default mt-1" disabled>{{$product['qty']}}</button>
		<a href="{{route('moreCart2',['id'=>$product['item']['id']])}}}" type="button" class="m-btn btn px-3 btn-primary mt-1"><i class="fa fa-plus"></i></a>
		</div>
	</td>
 <?php
        if($product['qty']){
            $qty[] = $product['qty'];
            Session::put('qty_items2',$qty);
     Session::save();
        }else{
           $qty="";
        }
	...
	if($product['price']){
        $price[] = $product['price'];
        Session::put('price_items2',$price);
        Session::save();
	}else{
	 $price="";
	}
...
<dl class="dlist-align">
    ...
    <dl class="dlist-align">
        ...
        @if(session::has('card2'))
                <?php $card  = Session::get('card2') ?>
    <dl class="dlist-align">
         <dt class="b" >Payment:</dt>
        <dd class="text-right text-warning b"> {{$card}} </dd>
        @endif
    </dl>
    @if(session::has('cash2'))
                <?php $cash = Session::get('cash2') ?>
    <dl class="dlist-align">
        <dt class="b" >Payment:</dt>
...
@if(Session::has('tbl2'))
    <dl class="dlist-align">
        ...
        @endif
              ...
              @if(Session::has('service2')&& Session::has('tbl2'))
<?php     $service = Session::get('service2');
 ?>
<dl class="dlist-align">
...
<div class="btn-group-vertical w-100" role="group" aria-label="Basic example">
    <div class="">
        ...
        <form method="post" action="{{route('tblpaid2')}}">
            @csrf
            @method('post')
            <div class="form-group">
    ...
    <button type="submit" class="btn mt-3 btn-success mysavebtn" onclick="">Paid</button>
        ...
        <div class="col-md-6 mb-0 pl-1">
            <a href="{{route('tblcard2')}}" class="btn py-3 btn-warning btn-lg btn-block"><i class="fa fa-credit-card"></i> Card </a>
        </div>
    </div>

    <div class="row pt-1 mt-0 mb-0 ">
        {{-- <div class="col-md-6 pt-0 mt-0 mb-0">
            <a href="{{route('printPos2')}}" class="btn py-3 btn-info btn-lg btn-block"><i class="fa fa-print"></i> Print </a>
        </div> --}}

            <div class="col-md-6 pt-0 mt-0 pr-1">
        <input type="button" value="Print" onclick="PrintDiv2();" id="btnPrint" class="btn py-3 btn-info btn-lg btn-block"/>
            </div>
        <div class="col-md-6 pt-0 mt-0 pl-1">
            <a href="{{route('destroy2')}}" class="btn py-3  btn-danger btn-lg btn-block"><i class="fa fa-times-circle "></i> Clear Cart </a>
        </div>
    </div>

    </div>
<div class="col-md-2 px-1">
<input type="button" value="Inst.P" onclick="PrintDiv2();" id="btnPrint" class="btn py-5 btn-success btn-lg btn-block"/>
</div>
</div>
    ...
    <script type="text/javascript">
        function PrintDiv2() {
            var contents = document.getElementById("printContent2").innerHTML;
