    ...
    if ( ! Session::has('tbl1')){
        ...
        ?>
         <div class="form-row mb-1">
            ...
            <div class="col">
            <a href="{{route('index1')}}">
            <button class="btn btn-warning btn-sm px-1 py-3 m-0"    style="width: 70px;font-size:20px;">1</button>
            </a>
            <?php
            if (Session::has('tbl1')){?>
...
<div class="btn-group-vertical w-200" role="group" aria-label="Basic example">
    <div class="">
        ...
        <form method="post" id="form2" action="{{route('order_tbl1')}}">
            @csrf
                  @method('post')
...
<figure class="media pt-2">
	...
	if($product['item']->name){
        $name_array[]= $product['item']->name;
         Session::put('name_items1',$name_array);
         Session::save();
	}else{
	    $name_array ='';
	}
	...
	if($product['price']){
        $price[] = $product['price'];
        Session::put('price_items1',$price);
        Session::save();
	}else{
	 $price ="";
	}
...
<dl class="dlist-align">
    ...
    <dl class="dlist-align">
        ...
        <dd class="text-right">-£{{number_format($extra,2)}}</dd>
                    ...
                    @if(session::has('card1'))
                <?php $card  = Session::get('card1') ?>
    <dl class="dlist-align">
         <dt class="b" >Payment:</dt>
        <dd class="text-right text-warning b"> {{$card}} </dd>
        @endif
    </dl>
    @if(session::has('cash1'))
                <?php $cash = Session::get('cash1') ?>
    <dl class="dlist-align">
        <dt class="b" >Payment:</dt>
...
@if(Session::has('tbl1'))
    <dl class="dlist-align">
        ...
        <!-- <dl class="dlist-align">
         ...
         @if(Session::has('service1')&& Session::has('tbl1'))
...
<dl class="dlist-align">
    ...
    <dl class="dlist-align">
        ...
        <dl class="dlist-align">
        <dt class="b h5 text-success">Paid:</dt>
   @if(session::has('cash1'))
        <?php if($pay){
    ?>
...
@if(session::has('card1') || session::has('cash1'))
            <div class="col-md-6 pt-0 mt-0 pr-1">
            <form action="{{route('printcard1')}}" id="myclearForm"></form>
        <input type="button" value="Print" id="btnPrint" onclick="PrintDivclr()" class="btn py-3 btn-info btn-lg btn-block"/>

            </div>
 @else
        <div class="col-md-6 pt-0 mt-0 pr-1">
        <button class="btn py-3 btn-info btn-lg btn-block" disabled>Print</button>
        </div>
        @endif
...
<input type="button" value="Inst.P" onclick="PrintDiv1();" id="btnPrint" class="btn py-5 btn-success btn-lg btn-block"/>
</div>
</div>
    ...
    <script type="text/javascript">
        function PrintDivclr() {
            ...
            setTimeout(function () {
                ...
                document.getElementById("myclearForm").submit();
